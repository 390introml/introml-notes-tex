...
\begin{itemize}
  \item The transition function is {\em stochastic}, \note{Recall that
          stochastic is another word for {\em probabilistic}; we don't say
          ``random'' because that can be interpreted in two ways, both of
          which are incorrect.  We don't pick the transition function itself
          at random from a distribution.  The transition function doesn't
          pick its output {\em uniformly} at random.}  meaning that it
        defines a probability distribution over the next state given the
        previous state and input, but each time it is evaluated it draws a
        new state from that distribution.
  \item We add the concept of a {\em reward} based on the action
        (input) taken when in any given state, enabling us to model
        situations where some states or state-action pairs are more
        desirable than others.
  \item The output is equal to the state (that is, $f_o$ in our state
        machine description is the identity function).
...
will have with the {\sc mdp}\note{In the finite-horizon case, we usually
  ignore the discount factor, equivalent to setting $\gamma$ to 1.}.
...
\begin{align}
  ...
  V^2_{\pi}(s) & = R(s, \pi(s)) + \sum_{s'}T(s, \pi(s), s')  V^1_{\pi}(s')       \\
  \vdots
  \nonumber                                                                      \\
  V^h_{\pi}(s) & = R(s, \pi(s)) + \sum_{s'}T(s, \pi(s), s')  V^{h - 1}_{\pi}(s')
  \label{eq:finite_value}
\end{align}
The sum over $s'$ is an {\em expected value}\index{expected value}: it
...
policy $\pi(s)$ assigns to arriving in state $s'$.

\question{What is $\sum_{s'} T(s, a, s')$ for any particular $s$ and
  $a$?  }
...
policy based on an expected finite-horizon undiscounted value:
\begin{equation}
...
\begin{examplebox}
  ...
  notation indicates an {\em expectation value}, i.e., an average
  ...
  \bigskip
  A very important point is that $R(s,a)$ is always deterministic for
  any given $s$ and $a$. Here $R_t$ represents the set of all possible
  $R(s_t,a)$ at step $t$; this $R_t$ is a random variable because
...
\begin{align}
  ...
  Q^2(s, a) & = R(s, a) + \sum_{s'}T(s, a, s') \max_{a'} Q^1(s', a')       \\
  \vdots
  \nonumber                                                                \\
...
$H$. Given $Q^h$, an optimal finite-horizon policy $\pi_h^*$ can be found as follows:
\begin{equation}
...
Once we know the optimal action-value function, then we can specify an
