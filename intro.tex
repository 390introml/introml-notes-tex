...
\begin{enumerate}
  ...
  \item {\bf Model type:}  Will an intermediate model be made?  What aspects of the data will be modeled?  How will the model be used to make predictions? \index{model}
...
For a regression problem, the training data $\dataTrain$ is in the form of a set of pairs:
\[\dataTrain = \{(\ex{x}{1},
...
model will be some hypothesis or prediction rule $y = h(x ; \Theta)$
for some functional form $h$.  The idea is that $\Theta$ is a set
of one or more parameter values\index{parameter} that will be determined by fitting the
model to the training data and then be held fixed during testing.  Given a new
...
where the loss function $\mathcal{L}(g, a)$ measures how bad it would be to make
