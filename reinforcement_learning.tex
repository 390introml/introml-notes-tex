...
So far, most of the learning problems we have looked at have been either
...
are told which value $\ex{y}{i}$ should be the output.
  {\em Reinforcement learning}\index{reinforcement learning} differs from
previous learning problems in several important ways:
...
Similar to MDPs, the learner is supposed to find a {\em policy}\index{reinforcement
  learning!policy}\index{policy!reinforcement learning}, mapping state $s$
...
what kind of hypothesis or model they learn. Roughly speaking, RL
methods can be categorized into model-free methods and model-based
methods. The main distinction lies in whether we are explicitly
learning the transition and reward models to assist the end-goal of
learning a policy.  We will start our discussion with the model-free
methods, and introduce two of the arguably most popular types of algorithms,
Q-learning (Section~\ref{sec-q_learning}) and policy search
(Section~\ref{sec-rl_policy_search}).  We then describe
model-based methods (Section~\ref{sec-rl_model_based}).  Finally, we
consider ``bandit'' problems (Section~\ref{sec-bandit}), which differ
...
methods (where value is a shorthand for value functions) and
policy-based methods. It's important to note that such
...
concentrates on learning (estimating) the state-action value function,
...
reward function $R$, and relies on samples to perform the updates.
\begin{codebox}
  \Procname{$\proc{Q-Learning}(\mathcal{S}, \mathcal{A}, s_0, \gamma,
      \alpha)$}\label{proc:Q_learn}
  ...
  \li     $Q[s, a] \leftarrow (1-\alpha)Q[s, a]
    + \alpha(r + \gamma \max_{a'}Q[s',a'])$
  \li $s \gets s'$
\end{codebox}

With the pseudocode provided for \nameref{proc:Q_learn}, there are a few
...
sampling the initial state at random. In any case, the inital state is necessary
...
more verbosely or specifically as tabular Q-learning.
...
our possible actions.\note{Here, the conditional distribution simply
  means that the chance of choosing an action depends on which state
  the agent is in. For example, suppose a robot is trying to get to a
  goal and can go left or right. An unconditional policy could say:
  regardless of my state I go left 99 percent of the time; a
  conditional policy would take into consideration the robot's
  state, and say: if I'm to the right of the goal, I go left 99
  percent of the time.}
