...
\times 1$ vector, so that
  ...
  a whole vector $z \in \R^K$ and generates as output a vector
$y \in (0, 1)^K$ with the property that $\sum_{j = 1}^K y_j = 1$,
  which means we can interpret it as a probability distribution over $K$ items:
  \[y = \textit{softmax}(z) =
    \begin{bmatrix}
    ...
    \end{bmatrix}\;\;.\]
  \question{Convince yourself that the vector of $y$ values will be
