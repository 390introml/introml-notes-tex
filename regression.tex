...
class of {\em linear regression} hypotheses\index{regression!linear regression}.  It is both simple to
...
{\em guess a whole bunch of different values of $\theta$ and
...
\question{If your data set has $n$ elements and the dimension of the
  $x$ values is $d$, what is the size of an individual $\ex{\theta}{i}$?}
\question{How do you think increasing $k$ will change the training
...
that minimizes mean squared error (this general problem is often
called {\em ordinary least squares} ({\sc ols})) is that we can find a
...
memorization, induced by the {\em loss} term, with generalization,
...
this can be realized using {\em ridge regression}.
...
Here, the notion of distance is quantified by the \textit{norm} \note{or more precisely, the $l_2$ norm} of
the parameter vector: for any $d$-dimensional vector $v \in \mathbb{R}^d,$ we have,
...
What if $\left(\Xt^T\Xt\right)$ is not invertible, as in the above example?
...
\end{equation}
and the term in front
\note{This is called ``ridge'' regression because we are adding a
  ``ridge'' of $n\lambda$ values along the diagonal of the matrix
  before inverting it.}
...
\begin{itemize}
  ...
  \item Evaluate resulting $h$ on a validation set {\em that does not
