...
one expects that
% AM_S2024 end
gradient descent (run long enough with small enough step size) will get very close to a local minimum point, though
we cannot guarantee that it will converge to a global minimum point.
% AM_S2024 begin
There are two notable exceptions to this common sense expectation: first, there are functions (even convex ones) with no minimum points,
like $f(x)=\exp(-x)$, for which gradient descent with a positive step size converges to $+\infty$. 
Second, gradient descent can get stagnated while approaching a point $x$ which is not a local minimum or maximum,
...
will lead to $x^{(k)}$ converging to zero as $k\to\infty$(check this!)
% AM_S2024 end
...
\question{Compute $\nabla_\theta \mathcal{L}_\text{ridge}(
    \theta^T x + \theta_0, y)$ by finding the
  vector of partial derivatives $(\partial \mathcal{L}_\text{ridge}(
    \theta^T x + \theta_0, y)/ \partial \theta_1, \ldots,
    \partial \mathcal{L}_\text{ridge}(
