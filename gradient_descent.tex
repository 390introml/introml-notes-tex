...
its gradient $f'$, we have to specify some hyper-parameters. These hyper-parameters include the initial value for parameter $\Theta$, a {\em step-size} hyper-parameter $\eta$, and an {\em accuracy} hyper-parameter~$\epsilon$\index{gradient descent!learning rate}.
...
gradient descent (run long enough with small enough step size) will get very close to a local minimum point, but
